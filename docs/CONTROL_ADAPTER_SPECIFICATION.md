...
SCPN Mirror Core — CONTROL adapter specification
-->

# CONTROL adapter specification

**Adapter identifier:** `scpn-mirror-core.control-adapter`  
...
magnetic-mirror adapter would supply device truth to the SCPN control
...
- channel identifier and physical quantity with SI unit — including axial
  field/density/potential profiles, end-loss analyser fluxes, and
  velocity-space population indicators with their exact definitions
  declared per channel;
- coordinate convention (machine-axis coordinate, expander region label)
  and spatial layout (scalar, axial array, or profile);
- sampling clock identity and timestamp semantics (quasi-steady time base
  with declared resolution for instability-burst channels);
...
- lifecycle phase (`field_ramp`, `build_up`, `sustained`, `termination`,
  `terminated`) with monotonic transition semantics and device-truth event
  records for velocity-space instability bursts and end-plug potential
  collapse;
- configuration identity: which registry configuration
  (`simple_magnetic_mirror`, `tandem_mirror`, or `gas_dynamic_mirror`),
  which cell/plugging layout, and which configuration policy revision
  produced the run;
...
For each actuator class (solenoid and mirror-coil currents, end-cell and
thermal-barrier heating systems, neutral-beam injectors, end-plate
potential-control electrodes, gas fuelling):
...
- machine-readable operational envelope (coil currents and forces, beam
  power, density and potential bounds, end-tank load limits with
  applicability domains);
- declared device-level hazard semantics for instability bursts and plug
  collapse;
...
above through CONTROL's real admission path: nominal quasi-steady runs,
plug-collapse sequences, boundary-of-envelope cases, invalid-payload
