...
| Pinned kernel library (`scpn-reactor-kernels` at one commit object) | every logarithm, exponential and power of the level-0 models comes from it, in Python and in the native crate; a substituted or drifted library would change numerics silently |
...
| Re-pinning the kernel library silently or inconsistently | the manifest, the `pyproject.toml` dependency, the installed package version, `rust/Cargo.toml`, `rust/Cargo.lock` and the CI install steps must name one commit object (contract test); the validator enforces the 40-hex commit, the 64-hex inventory digest and the consumed kernel identifiers; native parity runs in CI |
