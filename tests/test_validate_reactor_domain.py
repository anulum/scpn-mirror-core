...
# SCPN Mirror Core — reactor-domain validator tests
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"configurations": ["gas_dynamic_mirror", "gas_dynamic_mirror"]},
            "must be unique",
        ),
        (
...
def test_map_cross_check_rejects_assignment_and_pin_drift(tmp_path: Path) -> None:
    ...
    map_path.write_text(
        json.dumps(
            {
                "planned_repositories": ["SCPN-MIRROR-CORE"],
                "existing_repositories": [],
                "configuration_assignments": {
                    "conventional_tokamak": "SCPN-MIRROR-CORE"
                },
