...
# SCPN Mirror Core — open-format mesh exports

"""Binary STL, glTF 2.0 (GLB) and STEP exports of the device model.

The serialisers are the shared geometry kernels of the pinned kernel
library (``scpn_reactor_kernels.geometry.export``, kernel
``geometry_exports``, and ``cad_step_export`` for STEP); this module owns
only the device-side provenance: the document ``extras`` of the GLB carry
the model schema, the source digests, the model digest, the declared
midplane plasma radius, the declared field profile, the flux-tube profile
it implies, the aperture clearances, the segment count, the units and the
non-claims, so a viewer can show where the meshes came from and on what
declared field the flux tube rests. Coordinates are stored in metres;
float32 storage is a format constraint of both mesh containers and is
...
from scpn_mirror_core.geometry.cad import DeviceModelCAD
from scpn_mirror_core.geometry.model import DeviceModel3D
...
def glb_extras(model: DeviceModel3D) -> dict[str, Any]:
    ...
    return {
        ...
        "midplane_plasma_radius_m": record["midplane_plasma_radius_m"],
        "field_profile": record["field_profile"],
        "flux_tube_profile": record["flux_tube_profile"],
        "flux_tube_clearances": record["flux_tube_clearances"],
...
def write_step(path: Path, model: DeviceModelCAD) -> int:
    ...
    normalised deterministic header (ADR 0007).
