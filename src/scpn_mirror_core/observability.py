...
# SCPN Mirror Core — diagnostic and clock semantics
...
identities for the magnetic-mirror configurations this repository owns,
...
from scpn_mirror_core.configuration import ConsistencyFinding
...
INTERCHANGE_BAND_HZ: Final = (1.0e2, 1.0e5)
DRIVE_BAND_HZ: Final = (1.0e6, 2.0e11)

SHOT_DURATION_CEILING_S: Final = 1.0e01
...
_CLOCK_KEY: Final[dict[ObservabilityClass, str]] = {
    ObservabilityClass.DIRECT_CYCLIC: "clock_epoch",
...
_COMPATIBLE_CLOCK_KINDS: Final[dict[ObservabilityClass, frozenset[ClockKind]]] = {
    ObservabilityClass.DIRECT_CYCLIC: frozenset({ClockKind.FACILITY_MONOTONIC}),
...
_CYCLIC_CLASSES: Final = frozenset(
    {ObservabilityClass.DIRECT_CYCLIC, ObservabilityClass.DERIVED_CYCLIC}
)
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-MIRROR-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    ...
    CandidateProfile(
        ...
        phenomenon=("RF, microwave, or beam drive phase against a facility reference"),
        configurations=(
            "gas_dynamic_mirror",
            "simple_magnetic_mirror",
            "tandem_mirror",
        ),
        observability_class=ObservabilityClass.DIRECT_CYCLIC,
    ),
    CandidateProfile(
        candidate_id="open.equilibrium_and_loss",
        phenomenon=("equilibrium, loss-cone, end-loss, and steady-state metrics"),
        configurations=(
            "gas_dynamic_mirror",
            "simple_magnetic_mirror",
            "tandem_mirror",
    ...
    CandidateProfile(
        ...
        phenomenon=("resolved interchange, flute, or related instability mode"),
        configurations=(
            "gas_dynamic_mirror",
            "simple_magnetic_mirror",
            "tandem_mirror",
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-MIRROR-CORE.
    acquisition_start_s
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-MIRROR-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented mirror diagnostic practice;
            empty when every declaration sits in its typical band.
            Findings are advisory instruments, not machine claims.
        """
        findings: list[ConsistencyFinding] = []
        clocks_by_id = {clock.identifier: clock for clock in self.clocks}
        low, high = INTERCHANGE_BAND_HZ
        drive_low, drive_high = DRIVE_BAND_HZ
        for channel in self.channels:
            if channel.candidate_id == "open.resolved_interchange_mode" and not (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            "declared interchange/flute band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the mirror low-frequency "
                            "instability scale 0.1-100 kHz (Post 1987)"
                        ),
                    )
                )
            if channel.candidate_id == "open.drive_reference" and not (
                drive_low <= channel.max_signal_frequency_hz <= drive_high
            ):
                findings.append(
                    ConsistencyFinding(
                        field=f"channels[{channel.identifier}].max_signal_frequency_hz",
                        message=(
                            "declared drive frequency "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the ICRF-to-ECRH heating range "
                            "1 MHz-200 GHz used on mirror machines "
                            "(Post 1987)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the mirror steady-shot scale of up to "
                            "~10 s (Post 1987)"
                        ),
                    )
                )
